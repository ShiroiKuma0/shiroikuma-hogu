---
name: build-apk
description: Build the signed release APK with the buildRelease Gradle task, then deliver it automatically via the global /after-build skill (adb push if a phone is connected, else scp to skhw — no prompt). Always build first without asking for permission to build. Use whenever the user asks to build the app, build the APK, make a release build, or build and send to the phone for the 白い熊 防具 / shiroikuma-hogu Aegis fork.
---
