---
name: build-apk
description: Build the signed release APK with the buildRelease Gradle task, then always ask whether to scp it to skhw (first choice) or adb push it to the connected phone. Always build first without asking for permission to build — the ONLY question you ever ask is the transfer question afterward. Use whenever the user asks to build the app, build the APK, make a release build, or build and send to the phone for the 白い熊 防具 / shiroikuma-hogu Aegis fork.
---
...
> immediately. Do **not** ask "shall I build?" / "want me to run buildRelease?" — that
...
1. **Note the output filename.** Read the current version and build number:
   ...
   - The APK will be `shiroikuma-hogu_<VERSION_NAME>+<BUILD_NUMBER>_arm64-v8a.apk`, using the `BUILD_NUMBER` value **before** the build (the task bumps it afterward).
   - versionCode for that build = `VERSION_CODE * 10000 + BUILD_NUMBER`.

2. **Build** (needs JDK 17+ — the default `java` on this machine is JDK 11, so pin JDK 21):
   - `JAVA_HOME=/usr/lib/jvm/java-21-openjdk-amd64 ./gradlew buildRelease < /dev/null`
   ...
   - The release build runs R8/minify + `shrinkResources`, so it is slower than a debug build; that is expected.
...
This repo's `buildRelease` task (`app/build.gradle`) has **no** interactive prompt — it only builds,
...
## Versioning & naming (the +N scheme)

This fork keeps upstream Aegis's version and appends a fork build counter, so every sideloaded build
installs as an *upgrade* and never collides with the official Aegis:
...
- APK filename = `shiroikuma-hogu_<VERSION_NAME>+<BUILD_NUMBER>_arm64-v8a.apk`.

The version logic + the `buildRelease` task live in `app/build.gradle`; `BUILD_NUMBER` is the only
build-to-build edit and the task owns it (it auto-bumps after each build). **Never hand-edit a version
to reuse a number** — even a rebuild of the same change gets the next `+N`. The `_arm64-v8a` suffix just
names the deploy target; the APK has no native libs and is effectively universal.

## Signing

Release signing is non-interactive: `app/build.gradle` reads credentials from `keystore.properties`
(falling back to `SIGNING_*` env vars). `keystore.properties` (gitignored) points to this fork's own
keystore `~/.android-keystores/shiroikuma-hogu.jks` (alias `hogu`). If neither `keystore.properties`
nor the env vars are present the build is unsigned and the APK will not install. Upstream Aegis ships
**no** signing config in its release build type (CI / F-Droid signs it), so this signing block is one of
our fork customizations — preserve it across upstream rebases.

## Prerequisites

- **JDK 21** at `/usr/lib/jvm/java-21-openjdk-amd64` (AGP 8.10 needs JDK 17+; the machine default is 11).
- **Android SDK** at `/home/shiroikuma/android-sdk`, recorded in the gitignored `local.properties`
